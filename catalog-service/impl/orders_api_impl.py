"""Реализация OrdersApi — использует сгенерированный BaseOrdersApi."""
import os
...
def _to_uuid(id):
    return id if isinstance(id, UUID) else UUID(str(id))
...
def _apply_promo(db: Session, code: str, total_amount: Decimal) -> tuple[Decimal, Decimal, Optional[UUID]]:
    stmt = select(PromoCode).where(PromoCode.code == code)
    promo = db.exec(stmt).first()
    if not promo:
        raise HTTPException(status_code=422, detail={"error_code": "PROMO_CODE_INVALID", "message": "Promo code not found"})
    now = datetime.now(timezone.utc)
    if not promo.active or promo.current_uses >= promo.max_uses or now < promo.valid_from or now > promo.valid_until:
        raise HTTPException(status_code=422, detail={"error_code": "PROMO_CODE_INVALID", "message": "Promo expired"})
    if total_amount < promo.min_order_amount:
        raise HTTPException(status_code=422, detail={"error_code": "PROMO_CODE_MIN_AMOUNT", "message": "Order amount below minimum"})
    if promo.discount_type == PromoDiscountType.PERCENTAGE:
        discount = min(total_amount * promo.discount_value / 100, total_amount * Decimal("0.70"))
    else:
...
class OrdersApiImpl(BaseOrdersApi):
    async def create_order(
        ...
        if token_bearer_auth["role"].value == "SELLER":
            raise HTTPException(status_code=403, detail={"error_code": "ACCESS_DENIED", "message": "Sellers cannot create orders"})
        _check_order_rate_limit(db, token_bearer_auth["user_id"], UserOperationType.CREATE_ORDER)
        _check_active_orders(db, token_bearer_auth["user_id"])
        ...
        if insufficient:
            raise HTTPException(status_code=409, detail={"error_code": "INSUFFICIENT_STOCK", "message": "Insufficient stock", "details": {"items": insufficient}})
        total = Decimal("0")
        order_items_data = []
        for item in order_create.items:
            prod = products_map[item.product_id]
            total += prod.price * item.quantity
            order_items_data.append((item.product_id, item.quantity, prod.price))
        discount_amount = Decimal("0")
        promo_code_id = None
        if order_create.promo_code:
            total, discount_amount, promo_code_id = _apply_promo(db, order_create.promo_code, total)
            stmt = select(PromoCode).where(PromoCode.code == order_create.promo_code)
            promo = db.exec(stmt).first()
            promo.current_uses += 1
            db.add(promo)
        order = Order(user_id=token_bearer_auth["user_id"], status=OrderStatus.CREATED, promo_code_id=promo_code_id, total_amount=total, discount_amount=discount_amount)
        db.add(order)
        db.flush()
        for product_id, qty, price in order_items_data:
            db.add(OrderItem(order_id=order.id, product_id=product_id, quantity=qty, price_at_order=price))
            prod = products_map[product_id]
            prod.stock -= qty
            db.add(prod)
        db.add(UserOperation(user_id=token_bearer_auth["user_id"], operation_type=UserOperationType.CREATE_ORDER))
        db.commit()
        db.refresh(order)
        items = db.exec(select(OrderItem).where(OrderItem.order_id == order.id)).all()
        return OrderResponse(
            id=order.id, user_id=order.user_id, status=order.status.value, total_amount=order.total_amount,
    ...
    async def update_order(self, id: str, order_update: OrderUpdate, token_bearer_auth: dict = None, db: Session = None) -> OrderResponse:
        if db is None:
            db = SessionLocal()
        token_bearer_auth = token_bearer_auth or get_current_auth()
        if token_bearer_auth["role"].value == "SELLER":
        ...
        _check_order_rate_limit(db, token_bearer_auth["user_id"], UserOperationType.UPDATE_ORDER)
        ...
        for item in order_update.items:
            prod = products_map[item.product_id]
            total += prod.price * item.quantity
            db.add(OrderItem(order_id=order.id, product_id=item.product_id, quantity=item.quantity, price_at_order=prod.price))
        ...
        db.add(order)
        db.add(UserOperation(user_id=token_bearer_auth["user_id"], operation_type=UserOperationType.UPDATE_ORDER))
        db.commit()
        db.refresh(order)
        items = db.exec(select(OrderItem).where(OrderItem.order_id == order.id)).all()
        return OrderResponse(
    ...
    async def cancel_order(self, id: str, token_bearer_auth: dict = None, db: Session = None) -> None:
        if db is None:
            db = SessionLocal()
        token_bearer_auth = token_bearer_auth or get_current_auth()
        if token_bearer_auth["role"].value == "SELLER":
