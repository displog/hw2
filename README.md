...
- **pgAdmin:** http://localhost:5052 (admin@example.com / admin)
...
3. Дождаться запуска (Flyway, catalog-service). API: http://localhost:8002 (или IP хоста:8002, если доступ извне).
...
5. Таблицы в БД — через pgAdmin (http://localhost:5052) или `psql` к контейнеру postgres.
